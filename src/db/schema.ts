...
// Better Auth tables. Domain rows below carry the authenticated user's id. The
// ownership columns are temporarily nullable only so pre-auth legacy rows can be
// claimed by the pre-registered operator after deployment.
...
export const experienceCards = sqliteTable(
  "experience_cards",
  {
  ...
  (t) => [
    index("experience_cards_user_archived_idx").on(t.userId, t.archived),
  ]
);
...
export const jobPostings = sqliteTable(
  ...
  (t) => [
    index("job_postings_user_collected_idx").on(t.userId, t.collectedAt),
  ]
);
...
export const requirements = sqliteTable(
  ...
  (t) => [
    index("requirements_user_posting_idx").on(t.userId, t.jobPostingId),
  ]
);
...
export const matches = sqliteTable(
  ...
  (t) => [
    index("matches_user_requirement_idx").on(t.userId, t.requirementId),
    index("matches_user_card_idx").on(t.userId, t.cardId),
  ]
);
...
export const drafts = sqliteTable(
  ...
  (t) => [index("drafts_user_posting_idx").on(t.userId, t.jobPostingId)]
);
...
export const draftSentences = sqliteTable(
  ...
  (t) => [
    index("draft_sentences_user_draft_idx").on(t.userId, t.draftId),
...
export const draftSentenceSources = sqliteTable(
  "draft_sentence_sources",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    userId: text("user_id").references(() => authUsers.id, {
      onDelete: "restrict",
  ...
  (t) => [
    index("draft_sentence_sources_user_sentence_idx").on(
      t.userId,
      t.sentenceId
    ),
    index("draft_sentence_sources_user_card_idx").on(t.userId, t.cardId),
  ]
);

export const askbacks = sqliteTable(
  "askbacks",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    userId: text("user_id").references(() => authUsers.id, {
  ...
  (t) => [index("askbacks_user_posting_idx").on(t.userId, t.jobPostingId)]
);
...
export const interviewQuestions = sqliteTable(
  ...
  (t) => [
    index("interview_questions_user_posting_idx").on(
      t.userId,
      t.jobPostingId
    ),
  ]
...
export const interviewAnswerPoints = sqliteTable(
  ...
  (t) => [
    index("interview_answer_points_user_question_idx").on(
      t.userId,
      t.questionId
    ),
...
export const applications = sqliteTable(
  ...
  (t) => [
    index("applications_user_posting_idx").on(t.userId, t.jobPostingId),
  ]
);
