...
- [done] (2026-07-15) **개인 실사용 계정 보호**: Better Auth 기반 단일 소유자 로그인,
  최초 설정 코드, 공개 가입 차단, 프로필·비밀번호·활성 세션 관리를 추가했다. 모든 업무
  API와 페이지는 DB 세션으로 보호하며, HTTPS와 별도 Basic Auth를 운영 방어층으로 둔다.
- [open] (전환 트리거 충족 시) **SaaS 멀티테넌트 전환**: 개인 사용으로 서비스성을 먼저
  검증한다. 타인에게 공개하기로 결정한 시점에 Postgres, 인증 사용자와 전 업무 테이블의
  `user_id`, 모든 쿼리의 소유권 검사, 기존 데이터 이관·격리 테스트를 한 작업으로 도입한다.
  현재 단일 소유자 인증을 멀티테넌트 구조로 간주하지 않는다. 설계 방침은 PLAN.md §4.1.
