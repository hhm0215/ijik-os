...
- **다음 마일스톤:** 실제 문서 가져오기 브라우저 E2E를 마치고 `notClaimable` 점수 보정 정책을 확정·구현한 뒤, 실제 공고 분석을 거쳐 VPS 개인 실사용 릴리스를 판단한다.
- **차단 요소:** VPS 릴리스 전 실제 문서 가져오기 브라우저 E2E와 `notClaimable` 점수·판정 코드 보정 및 회귀 검증이 필요하다. Windows는 Ollama 0.31 이상에서 검증한다.
