...
> 이 프로젝트 자체가 교재다. "하네스 엔지니어링"이란 AI 에이전트(Claude Code 등)가
...
Claude Code는 세션 시작 시 프로젝트 루트의 `CLAUDE.md`를 자동으로 읽는다.
...
**설계 포인트:**
- 원칙: 읽기·빌드·테스트는 넓게 허용, 파괴적인 것(rm, push, 배포)은 확인 요구
- 우리 설정: npm/drizzle/sqlite3(로컬 DB 조회)/ollama는 자유, `data/` 삭제는 deny
