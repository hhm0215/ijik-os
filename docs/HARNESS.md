...
코드로 강제할 수 있는 정책은 `npm test`의 빠른 단위 테스트로 먼저 고정한다. Node의
내장 테스트 러너에 `tsx`를 연결해 TypeScript 테스트를 Mac·Windows에서 같은 명령으로
실행하며, LLM을 실제 호출하는 느린 품질 평가는 `pipeline-eval`로 분리한다.
