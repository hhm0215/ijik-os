...
| 파일 | 역할 | 언제 읽나 | 언제 쓰나 |
|---|---|---|---|
| `docs/PLAN.md` | 비전·원칙·로드맵·결정 로그 | 방향/범위 판단이 필요할 때 | 중요한 결정이 내려졌을 때 (결정 로그 추가) |
| `docs/PROGRESS.md` | 진행 상태 + 세션 로그 | **세션 시작 시** | 세션 마무리 시 (`/wrap-session`) |
| `IDEAS.md` | 아이디어/피드백 큐 | **세션 시작 시** (open 항목 확인) | 새 아이디어·발견 즉시, 상태 변경 시 |
| `docs/HARNESS.md` | 하네스 학습 가이드 | 하네스 구조 질문을 받을 때 | 하네스 구성이 바뀔 때 |
...
- **저장소 경계:** `ijik-os`는 실제 제품을 위한 독립 Next.js 풀스택 앱이다.
